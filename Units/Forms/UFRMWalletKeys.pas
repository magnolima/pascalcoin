...
Var
    ...
    index: Integer;
begin
    if not GetSelectedWalletKeyAndIndex(wk, index) then
        exit;
    s := wk.Name;
    if InputQuery('Change name', 'Input new key name:', s) then
    begin
...
begin
    ...
    if Application.MessageBox(Pchar(s), Pchar('Delete key'), MB_YESNO + MB_DEFBUTTON2 + MB_ICONWARNING) <> Idyes then
        exit;
    if Application.MessageBox(Pchar('Are you sure you want to delete?'), Pchar('Delete key'), MB_YESNO + MB_DEFBUTTON2 + MB_ICONWARNING) <> Idyes then
        exit;
...
Var
    ...
    fs: TFileStream;
begin
    if WalletKeys.Count <= 0 then
        raise Exception.Create('Your wallet is empty. No keys!');
    if ((WalletKeys.IsValidPassword) And (WalletKeys.WalletPassword = '')) then
    begin
        if Application.MessageBox(Pchar('Your wallet has NO PASSWORD' + #10 + #10 +
          'It is recommend to protect your wallet with a password prior to exporting it!' + #10 + #10 + 'Continue without password protection?'),
    ...
    begin
        if Application.MessageBox(Pchar('This file exists:' + #10 + efn + #10#10 + 'Overwrite?'), Pchar(Application.Title),
...
begin
    ...
    if Not GetSelectedWalletKey(wk) then
        exit;
    if Assigned(wk.PrivateKey) then
    begin
        if InputQuery('Export private key', 'Insert a password to export', pwd1) then
        begin
            if InputQuery('Export private key', 'Repeat the password to export', pwd2) then
            begin
                if pwd1 <> pwd2 then
                    raise Exception.Create('Passwords does not match!');
                enc := TCrypto.ToHexaString(TAESComp.EVP_Encrypt_AES256(wk.PrivateKey.ExportToRaw, pwd1));
                Clipboard.AsText := enc;
                Application.MessageBox(Pchar('The password has been encrypted with your password and copied to the clipboard.' + #10 + #10 + 'Password: "' +
                  pwd1 + '"' + #10 + #10 + 'Encrypted key value (Copied to the clipboard):' + #10 + enc + #10 + #10 + 'Length=' + Inttostr(length(enc)) +
                  ' bytes'), Pchar(Application.Title), MB_OK + MB_ICONINFORMATION);
            end
            else
                raise Exception.Create('Cancelled operation');
        end;
    end;
end;
...
begin
    ...
    Clipboard.AsText := s;
    Application.MessageBox(Pchar('The public key has been copied to the clipboard' + #10 + #10 + 'Public key value:' + #10 + s + #10 + #10 + 'Length=' +
      Inttostr(length(s)) + ' bytes'), Pchar(Application.Title), MB_OK + MB_ICONINFORMATION);
end;
...
begin
    ...
    if Not FileExists(ifn) then
        raise Exception.Create('Cannot find file ' + ifn);
    ...
    try
        ...
        pwd := '';
        While (Not wki.IsValidPassword) do
        begin
            if Not InputQuery('Import', 'Enter the wallet file password:', pwd) then
                exit;
            wki.WalletPassword := pwd;
            if not wki.IsValidPassword then
            begin
                If Application.MessageBox(Pchar('Password entered is not valid, retry?'), Pchar(Application.Title), MB_ICONERROR + MB_YESNO) <> Idyes then
                    exit;
            end;
        end;
        cPrivatekeys := 0;
        cPublicKeys := 0;
        if wki.IsValidPassword then
        begin
            for i := 0 to wki.Count - 1 do
            begin
                if WalletKeys.IndexOfAccountKey(wki.Key[i].AccountKey) < 0 then
                begin
                    If Assigned(wki.Key[i].PrivateKey) then
                    begin
        ...
        if (cPrivatekeys > 0) Or (cPublicKeys > 0) then
        begin
            Application.MessageBox(Pchar('Wallet file imported successfully' + #10 + #10 + 'File:' + ifn + #10 + #10 + 'Total keys in wallet: ' +
              Inttostr(wki.Count) + #10 + 'Imported private keys: ' + Inttostr(cPrivatekeys) + #10 + 'Imported public keys only: ' + Inttostr(cPublicKeys) + #10
              + 'Duplicated (not imported): ' + Inttostr(wki.Count - cPrivatekeys - cPublicKeys)), Pchar(Application.Title), MB_OK + MB_ICONINFORMATION);
        end
        else
        begin
            Application.MessageBox(Pchar('Wallet file keys were already in your wallet. Nothing imported' + #10 + #10 + 'File:' + ifn + #10 + #10 +
              'Total keys in wallet: ' + Inttostr(wki.Count)), Pchar(Application.Title), MB_OK + MB_ICONWARNING);
        end;
    finally
        wki.Free;
    end;
...
var
    ...
    begin
        Repeat
            s := '';
            desenc := '';
            if InputQuery('Import private key', 'Enter the password:', s) then
            begin
                If (TAESComp.EVP_Decrypt_AES256(enc, s, desenc)) then
                begin
                    if (desenc <> '') then
                    begin
                    ...
                    begin
                        if Application.MessageBox(Pchar('Invalid password!'), '', MB_RETRYCANCEL + MB_ICONERROR) <> IDRETRY then
                        begin
                ...
                begin
                    if Application.MessageBox(Pchar('Invalid password or corrupted data!'), '', MB_RETRYCANCEL + MB_ICONERROR) <> IDRETRY then
                    begin
                        ParseEncryptedKey := False;
                        exit;
                    end;
...
begin
    EC := Nil;
    CheckIsWalletKeyValidPassword;
    if Not Assigned(WalletKeys) then
        exit;
    if InputQuery('Import private key', 'Insert the password protected private key or raw private key', s) then
    begin
        s := trim(s);
        if (s = '') then
            raise Exception.Create('No valid key');
        enc := TCrypto.HexaToRaw(s);
        if (enc = '') then
            raise Exception.Create('Invalid text... You must enter an hexadecimal value ("0".."9" or "A".."F")');
        case length(enc) of
            32:
                parseResult := ParseRawKey(CT_NID_secp256k1);
            35, 36:
                parseResult := ParseRawKey(CT_NID_sect283k1);
            48:
                parseResult := ParseRawKey(CT_NID_secp384r1);
            65, 66:
                parseResult := ParseRawKey(CT_NID_secp521r1);
            64, 80, 96:
                parseResult := ParseEncryptedKey;
        else
            Exception.Create('Invalidly formatted private key string. Ensure it is an encrypted private key export or raw private key hexstring.');
        end;
        if (parseResult = False) then
            exit;
        Try
            // EC is assigned by ParseRawKey/ImportEncryptedKey
            if Not Assigned(EC) then
            begin
            ...
            begin
                wk := WalletKeys.Key[i];
                if Assigned(wk.PrivateKey) And (Assigned(wk.PrivateKey.PrivateKey)) then
...
begin
    ...
    if Not Assigned(WalletKeys) then
        exit;
    if Not InputQuery('Import publick key', 'Insert the public key in hexa format or imported format', s) then
        exit;
    If not TAccountComp.AccountPublicKeyImport(s, account, errors) then
    begin
    ...
    If not TAccountComp.IsValidAccountKey(account, errors) then
        raise Exception.Create('This data is not a valid public key' + #10 + errors);
    if WalletKeys.IndexOfAccountKey(account) >= 0 then
        raise Exception.Create('This key exists on your wallet');
    s := 'Imported public key ' + DateTimeToStr(now);
    if InputQuery('Set a name', 'Name for this private key:', s) then
    begin
...
Var
    s, s2: String;
begin
    if FWalletKeys.IsValidPassword then
    begin
        s := '';
        s2 := '';
        if Not InputQuery('Change password', 'Enter new password', s) then
            exit;
        if trim(s) <> s then
            raise Exception.Create('Password cannot start or end with a space character');
        if Not InputQuery('Change password', 'Enter new password again', s2) then
            exit;
        if s <> s2 then
            raise Exception.Create('Two passwords are different!');

        FWalletKeys.WalletPassword := s;
        Application.MessageBox(Pchar('Password changed!' + #10 + #10 + 'Please note that your new password is "' + s + '"' + #10 + #10 +
          '(If you lose this password, you will lose your wallet forever!)'), Pchar(Application.Title), MB_ICONWARNING + MB_OK);
        UpdateWalletKeys;
    end
    else
    begin
        s := '';
        Repeat
            if Not InputQuery('Wallet password', 'Enter wallet password', s) then
                exit;
            FWalletKeys.WalletPassword := s;
            if Not FWalletKeys.IsValidPassword then
                Application.MessageBox(Pchar('Invalid password'), Pchar(Application.Title), MB_ICONERROR + MB_OK);
...
begin
    if Not Assigned(FWalletKeys) then
        exit;

    if Not FWalletKeys.IsValidPassword then
    begin
        Application.MessageBox(Pchar('Wallet key is encrypted!' + #10 + #10 + 'You must enter password to continue...'), Pchar(Application.Title),
          MB_OK + MB_ICONWARNING);
        bbUpdatePasswordClick(Nil);
        if Not FWalletKeys.IsValidPassword then
            raise Exception.Create('Cannot continue without valid password');
    end;
end;
...
begin
    if Assigned(FWalletKeys) then
        FWalletKeys.OnChanged := FOldOnChanged;
    inherited;
end;
...
begin
    ...
    if Not Assigned(WalletKeys) then
        exit;
    if lbWalletKeys.ItemIndex < 0 then
        exit;
    index := Integer(lbWalletKeys.Items.Objects[lbWalletKeys.ItemIndex]);
    if (index >= 0) And (index < WalletKeys.Count) then
    begin
...
begin
    ...
    if Assigned(FOldOnChanged) then
        FOldOnChanged(Sender);
end;

procedure TFRMWalletKeys.SetWalletKeys(const Value: TWalletKeys);
begin
    if FWalletKeys = Value then
        exit;
    if Assigned(FWalletKeys) then
        FWalletKeys.OnChanged := FOldOnChanged;
    FWalletKeys := Value;
...
begin
    ...
    try
        ...
        if wk.Name = '' then
            lblKeyName.Caption := '(No name)'
        else
            lblKeyName.Caption := wk.Name;
        if Assigned(wk.PrivateKey) then
        begin
...
begin
    ...
    Try
        ...
        If FWalletKeys.IsValidPassword then
        begin
            if FWalletKeys.WalletPassword = '' then
                lblKeysEncrypted.Caption := 'Wallet without password'
            else
                lblKeysEncrypted.Caption := 'Wallet is password protected';
        ...
        for i := 0 to WalletKeys.Count - 1 do
        begin
            wk := WalletKeys.Key[i];
            if (wk.Name = '') then
            begin
